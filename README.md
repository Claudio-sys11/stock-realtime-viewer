...
- **현재 시간(한국시간)** 상단 표시
- **장 운영시간**: 각 지수 아래에 장시작·마감 + 전·후 경과(시작 전=빨강/후=파랑, 시작 전엔 마감 숨김). **코스피는 KRX·NXT 따로 표시**. **마감 30분 전부터 빨간 네온 깜빡임**
...
- **종목 검색**: 국내(삼성전자/005930)뿐 아니라 **미국 주식(apple/AAPL/테슬라 등)** 도 검색해 추가 — 미국 종목은 달러 아래 **원화 환산(현재 환율)** 회색 표시
